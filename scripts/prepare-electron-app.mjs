...
function rebrandDevelopmentBundle() {
  ...
  rewritePlist(join(contents, "Info.plist"), {
    ...
    CFBundleIconFile: "maximo-syntax.icns",
  ...
  copyFileSync(resolve(process.cwd(), "assets", "app-icon.icns"), join(contents, "Resources", "maximo-syntax.icns"));
}
...
export function getElectronExecutable() {
  ...
  if (!existsSync(TARGET_APP) || !existsSync(targetExecutable) || cachedFingerprint !== fingerprint) {
    ...
    copyFileSync(resolve(process.cwd(), "assets", "app-icon.icns"), join(TARGET_APP, "Contents", "Resources", "maximo-syntax.icns"));
  }
