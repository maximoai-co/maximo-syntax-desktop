...
private func optionKeyIsDown(_ keyCode: CGKeyCode) -> Bool {
...
final class OptionChordMonitor {
    ...
    fileprivate func handleEvent(type: CGEventType, event: CGEvent) {
        ...
        // Hardware key state is authoritative. Device-specific 0x20/0x40 flags
        // are missing on some keyboards and macOS versions, which made the chord
        // silently never fire even when Input Monitoring was granted.
        let leftDown = optionKeyIsDown(leftOptionKeyCode) || event.flags.contains(leftOptionDeviceFlagMask)
        let rightDown = optionKeyIsDown(rightOptionKeyCode) || event.flags.contains(rightOptionDeviceFlagMask)

        if !event.flags.contains(.maskAlternate), !leftDown, !rightDown {
            resetChordState()
            return
        }

        leftOptionIsDown = leftDown
        rightOptionIsDown = rightDown
