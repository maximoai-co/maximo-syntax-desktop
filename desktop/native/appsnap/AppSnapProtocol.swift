...
struct AppSnapOptions {
    ...
    static func parse(_ arguments: [String]) throws -> AppSnapOptions {
        ...
        while index < arguments.count {
            ...
            switch argument {
            case "--check-permissions", "--request-permissions", "--request-screen-recording", "--request-input-monitoring", "--watch":
        ...
        case "--request-screen-recording":
            ...
            return AppSnapOptions(mode: .requestScreenRecording)
        case "--request-input-monitoring":
            guard outputDirectory == nil, excludedBundleIdentifier == nil, !externalTrigger else {
                throw AppSnapFailure(
                    code: "invalid_arguments",
                    message: "Permission requests do not accept watch arguments."
            ...
            return AppSnapOptions(mode: .requestInputMonitoring)
        case "--watch":
        ...
        default:
            throw AppSnapFailure(
                code: "invalid_arguments",
                message: "Expected --check-permissions, --request-permissions, --request-screen-recording, --request-input-monitoring, or --watch."
            )
        }
    }
}
...
final class NDJSONEmitter {
    ...
    private func writeDiagnostic(_ message: String) {
        guard let data = "[maximo-syntax-appsnap-helper] \(message)\n".data(using: .utf8) else {
            return
        }
        lock.lock()
        defer { lock.unlock() }
