    ...
    if application.bundleIdentifier == excludedBundleIdentifier {
        return .failure(
            AppSnapFailure(
                ...
                message: "Maximo Syntax cannot capture its own window."
            )
        )
    }
...
final class OneFrameWindowCapture: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    private let outputQueue = DispatchQueue(label: "co.maximoai.syntax.appsnap.stream-output")
...
final class AppSnapCaptureCoordinator {
    ...
    private let queue = DispatchQueue(label: "co.maximoai.syntax.appsnap.capture")
