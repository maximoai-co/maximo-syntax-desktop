...
import { MAX_ATTACHMENT_COUNT } from "./types";
import {
  DesktopAppSnapManager,
  desktopAppSnapPlatform,
  isPathInsideDirectory,
  parseAppSnapHelperMessage,
} from "./app-snap-manager";

const EXCLUDED_BUNDLE_ID = "com.maximoai.syntax.desktop";
...
describe("desktop AppSnap platform state", () => {
  ...
  it("asks the host app for Screen Recording when enabling AppSnap", async () => {
    ...
    const spawn = vi
      ...
      .mockReturnValueOnce(screenChild) as unknown as typeof ChildProcess.spawn;
    const manager = new DesktopAppSnapManager({
      platform: "darwin",
...
describe("AppSnap shortcut availability", () => {
  ...
  it("lets the renderer trigger a capture over the helper stdin", async () => {
    const permissionChild = createFakeChildProcess();
    const watchChild = createFakeChildProcess();
    const spawn = vi
      .fn()
    ...
    const manager = new DesktopAppSnapManager({
      platform: "darwin",
      helperPath: process.execPath,
      captureDirectory: "/tmp/maximo-appsnap-test",
      excludedBundleId: EXCLUDED_BUNDLE_ID,
      spawn,
      onState: vi.fn(),
      onCaptured: vi.fn(),
    ...
    expect(manager.triggerCapture()).toBe(true);
    expect(watchChild.stdin.read()?.toString()).toBe("trigger\n");
    manager.dispose();
  });
});
...
describe("AppSnap helper protocol", () => {
  ...
  it("retains a full composer batch and reports any overflow", async () => {
    ...
    try {
      ...
      for (let index = 0; index <= MAX_ATTACHMENT_COUNT; index += 1) {
      ...
      await vi.waitFor(() => {
        expect(onCaptured).toHaveBeenCalledTimes(MAX_ATTACHMENT_COUNT + 1);
      });
      const pendingCaptures = await manager.listPendingCaptures();
      expect(pendingCaptures).toHaveLength(MAX_ATTACHMENT_COUNT);
