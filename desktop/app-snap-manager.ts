...
import { openAppSnapPrivacySettings } from "./app-snap-ipc.js";
import {
  ...
  isAppSnapShortcut,
  sameAppSnapShortcut,
} from "./app-snap-shortcut.js";

const MAX_PENDING_CAPTURES = MAX_ATTACHMENT_COUNT;
...
type AppSnapPermissionPhase = "screen" | "input" | "done";
const PERMISSION_PHASE_FILE = "permission-phase.json";
...
export interface DesktopAppSnapManagerOptions {
  ...
  requestHostInputMonitoring?: () => boolean;
  getHostScreenRecordingPermission?: () => DesktopAppSnapPermission;
  requestHostScreenRecording?: () => Promise<DesktopAppSnapPermission> | DesktopAppSnapPermission;
  onPermissionPrompt?: (pane: "screen" | "input") => void;
}
...
function parseStoredPendingCapture(value: unknown): StoredPendingAppSnapCapture | null {
  ...
  if (
    ...
    sizeBytes <= 0 ||
    sizeBytes > MAX_APPSNAP_IMAGE_BYTES
  ) {
    return null;
  }
  return {
...
async function readValidatedPendingPng(filePath: string, expectedBytes?: number): Promise<Buffer> {
  const bytes = await readRegularFile(filePath, MAX_APPSNAP_IMAGE_BYTES, expectedBytes);
...
export class DesktopAppSnapManager {
  ...
  getState(): DesktopAppSnapState {
    this.#applyHostPermissionOverlay();
    return {
  ...
  async setEnabled(enabled: boolean): Promise<DesktopAppSnapState> {
    ...
    if (!enabled) {
      ...
      this.#writePermissionPhase("screen");
  ...
  async requestInputMonitoring(): Promise<DesktopAppSnapState> {
    if (this.#platform !== "macos" || this.#disposed) return this.getState();
    this.#writePermissionPhase("input");
    await this.#runPermissionCommand("--request-input-monitoring");
    this.#writePermissionPhase("done");
    this.#options.onPermissionPrompt?.("input");
    await openAppSnapPrivacySettings("input");
    if (!(await this.#runPermissionCommand("--check-permissions"))) return this.getState();
    await this.#reconcileWatchProcess();
    return this.getState();
  }
  ...
  triggerCapture(): boolean {
    if (this.#disposed || !this.#enabled || !this.#watchProcess) return false;
    try {
  ...
  #applyHostPermissionOverlay(): void {
    if (this.#options.getHostScreenRecordingPermission?.() === "granted") {
      this.#screenRecordingPermission = "granted";
    }
  }

  #permissionPhasePath(): string {
    return Path.join(this.#options.captureDirectory, PERMISSION_PHASE_FILE);
  }

  #readPermissionPhase(): AppSnapPermissionPhase {
    try {
      const parsed = JSON.parse(FS.readFileSync(this.#permissionPhasePath(), "utf8")) as { phase?: string };
      if (parsed.phase === "screen" || parsed.phase === "input" || parsed.phase === "done") return parsed.phase;
    } catch {
  ...
  #writePermissionPhase(phase: AppSnapPermissionPhase): void {
    try {
      FS.mkdirSync(Path.dirname(this.#permissionPhasePath()), { recursive: true, mode: 0o700 });
      FS.writeFileSync(this.#permissionPhasePath(), `${JSON.stringify({ phase })}\n`, { mode: 0o600 });
    } catch {
      // The next launch may repeat a prompt.
    }
  }

  #permissionPrompt(): "screen" | "input" | null {
    if (!this.#enabled || this.#platform !== "macos") return null;
    const phase = this.#readPermissionPhase();
    if (phase === "input" && this.#inputMonitoringPermission !== "granted") return "input";
    if (this.#screenRecordingPermission !== "granted") return "screen";
    if (this.#inputMonitoringPermission !== "granted") return "input";
    return null;
  }

  async #advancePermissionPrompt(): Promise<void> {
    this.#applyHostPermissionOverlay();
    const phase = this.#readPermissionPhase();
    const screenGranted = this.#screenRecordingPermission === "granted";
    if (phase !== "input" && phase !== "done" && !screenGranted) {
      await this.#options.requestHostScreenRecording?.();
      this.#applyHostPermissionOverlay();
      await this.#runPermissionCommand("--request-screen-recording");
      this.#writePermissionPhase("input");
      this.#options.onPermissionPrompt?.("screen");
      await openAppSnapPrivacySettings("screen");
      return;
    }
    if (this.#inputMonitoringPermission !== "granted") {
      await this.#runPermissionCommand("--request-input-monitoring");
      this.#writePermissionPhase("done");
      this.#options.onPermissionPrompt?.("input");
      await openAppSnapPrivacySettings("input");
  ...
  #handleWatchMessage(child: AppSnapHelperProcess, message: AppSnapHelperMessage): void {
    ...
    // Benign overlap errors surface as a toast without yanking Maximo Syntax to the
  ...
  async #consumeCapture(
    ...
    if (discardedRecord) {
      ...
      this.#emitCaptureError(
        ...
        `Maximo Syntax could retain only the latest ${MAX_PENDING_CAPTURES} AppSnaps while the composer was unavailable. The oldest capture was discarded.`,
